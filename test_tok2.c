...
int is_space(int64_t ch) { return ch==32||ch==9||ch==10||ch==13; }
...
int main() {
    ...
    // Skip "函数" (2 chars, 6 bytes)
    pos += rt_utf8_byte_length(cur_char);
    cur_char = rt_utf8_codepoint_at(src, pos);
    pos += rt_utf8_byte_length(cur_char);
    cur_char = rt_utf8_codepoint_at(src, pos);
    
    printf("After 函数, char=%lld (space=%d)\n", cur_char, cur_char==32);
    
    clock_t start = clock();
    int count = 0;
    while (is_space(cur_char) && pos < len) {
    ...
    printf("Final pos=%lld, cur_char=%lld ('%c')\n", pos, cur_char, (char)cur_char);
    free(src); return 0;
}
