...
| **调试器 (debugger)**  |  ✅  | --debug 断点陷阱 + 交互式单步/继续/退出                       |
| **自举 (compiler_v2)** |  ✅  | **XY 自身实现，自举验证通过**                                 |
| **测试用例**           |  ✅  | 单元/集成/自举测试全部通过 (121)                              |
| **发布打包 (build)**   |  ✅  | 三平台发布脚本 + CI release workflow                          |
...
# 调试模式：注入断点陷阱，支持交互式单步/继续/退出
cargo run -- examples/hello.xy --debug --build
./output.exe  # 进入 (xy-dbg) 交互提示符
```
...
模式匹配支持对枚举变体进行多路分支判断，支持字段绑定和通配默认分支。
